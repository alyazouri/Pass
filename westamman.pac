...
var SESSION = {
  ...
  lobbyWide:          false,
  jordanIPSeen:       0,
  westAmmanIPSeen:    0,   // ← تتبّع نطاقات غرب عمّان
  totalIPSeen:        0,
  txCount:            0,
  hashSeed:           0
};
...
var WEST_AMMAN_DENSITY_MIN = 0.40;  // ← 40% على الأقل من غرب عمّان
...
//  المصدر: تخصيصات Orange JO + Zain + Umniah
//  في مناطق: عبدون، الصويفية، خلدا، دير غبار،
//  الدوار الرابع→السابع، تلاع العلي، الجبيهة،
//  شميساني، أم أذينة
// ─────────────────────────────────────────────
var WEST_AMMAN_IPV4_CIDR = [
  // Orange Jordan (JTC) — غرب عمّان وعبدون والدوارات
  "176.29.128.0/18",
  "176.29.192.0/19",
  "176.29.224.0/20",
  "176.28.128.0/19",
  ...
  // Batelco / Mada — مناطق راقية متفرقة
  "94.142.32.0/21",
  "94.142.40.0/22",
  "94.249.0.0/19",
  "94.249.32.0/20",

  // نطاقات ألياف متخصصة (FTTH) — عبدون / دير غبار
  "5.45.128.0/21",
  "5.45.136.0/22",
  "176.29.152.0/22",
  "176.29.153.0/24"
...
// ─────────────────────────────────────────────
//  §8-C  KNOWN JO PREFIXES (fast-path)
// ─────────────────────────────────────────────
...
function isInIPv6Prefix(ip, prefixList) {
  ...
  for (var i = 0; i < prefixList.length; i++) {
    if (s.indexOf(prefixList[i].toLowerCase()) === 0) return true;
  }
  return false;
}
...
// ─────────────────────────────────────────────
//  §11  DENSITY TRACKER — Jordan + West Amman
// ─────────────────────────────────────────────
...
//  jordanLobbyForce — منطق غرب عمّان
//  الأولوية: West Amman → Jordan → BLOCK
function jordanLobbyForce(ip, host, t) {
  trackDensity(ip);

  // IP غرب عمّان → تمرير فوري عبر البروكسي
  if (isWestAmmanIP(ip)) {
    if (SESSION.state === STATE.IDLE) transitionTo(STATE.SEARCHING, t);
    return pickAliveProxy(T2_LOBBY, host);
  }

  // IP أردني عادي → تمرير فقط إذا كانت كثافة غرب عمّان كافية
  if (isJordanIP(ip)) {
...
//  §15  FindProxyForURL
//  منطق القرار:
//  1) IP مباشر → تحقق أردني → West Amman density
//  2) Hostname PUBG → Known JO fast-path
//  3) DNS resolve → West Amman first → Jordan fallback
//  4) LOBBY دائماً عبر routeLobbyTraffic (لا DIRECT)
// ─────────────────────────────────────────────
function FindProxyForURL(url, host) {
  ...
  // ابحث عن IP غرب عمّان أولاً، ثم أردني عام
  var waIP   = firstWestAmmanIP(ips);
  var bestIP = waIP || firstJordanIP(ips);
  ...
  if (ttype === "LOBBY" || ttype === "SOCIAL" || ttype === "UNKNOWN") {
    ...
    // [FIX] لا DIRECT — يُوجَّه عبر البروكسي دائماً
    return routeLobbyTraffic(host, t);
  }
