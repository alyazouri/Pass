...
// تحقق من أن الـ host هو IP مباشر (IPv4 أو IPv6)
function isIPAddress(host) {
  return /^(\d{1,3}\.){3}\d{1,3}$/.test(host) || host.indexOf(":") > -1;
}
...
// يُعطي الأولوية لـ IPv4 لأن isInNet أكثر دقة
...
function FindProxyForURL(url, host) {
  ...
  var isDirectIP = isIPAddress(host);

  // -------------------------------------------------------
  // المسار الأول: الـ host عبارة عن IP مباشر (بدون DNS)
  // -------------------------------------------------------
  if (isDirectIP) {
    var isJordan = isIPv6(host)
      ? inV6List(host, JORDAN_IPV6)
      : inV4List(host, JORDAN_IPV4);

    // IP خارج الأردن — لا علاقة له بالسكربت
    ...
    // IP أردني بدون تصنيف واضح — نمرره عبر اللوبي احتياطاً
    return lobbyProxy();
  }

  // -------------------------------------------------------
  // المسار الثاني: الـ host عبارة عن domain name
  // -------------------------------------------------------
